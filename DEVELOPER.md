...
* `develop` — рабочая интеграционная ветка. Признаки `feature/*` / `fix/*` заводятся от неё.
* `main` — стабильная и default-ветка (посадочная страница репозитория): в неё
  попадают готовые изменения из `develop` (merge по готовности к релизу).
