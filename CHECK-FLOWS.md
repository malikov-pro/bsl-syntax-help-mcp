...
Слой (`layer`) — полный снимок справки одной версии: `base` = 8.3.24 целиком,
`8.3.25`…`8.5.1` = свои версии целиком (`Layers.java`: `versionFor`;
`app/versions.py` ↔ чекбоксы в плагине — править только парой). Дельт между
версиями нет: каждый слой кладётся в `document_bodies`/`layer_members` целиком.
Видимость при поиске = членство в слое запрошенной версии (8.3.8–8.3.24 →
`base`), фильтр по `introduced_in`; тело документа — из самого свежего слоя не
новее запрошенной версии (`_visible_docs`, `pick_body_layer` в `search.py`).
